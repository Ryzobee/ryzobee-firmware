# RyzoBee RootMaker firmware — V0.10.1
...
**全量项目烧录会覆盖 scripts 分区及用户脚本、自启动副本。** 先另存需要保留的源码；标准流程不擦 NVS，不等于恢复出厂设置。不要仅为升级而擦除整片 Flash。连接失败先检查端口、占用和 DTR/RTS 下载链路；详见顶层 README。
